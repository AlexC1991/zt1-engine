...
Pallet *PalletManager::getPallet(std::string &file_name) {
  ...
  SDL_Log(
    "Could not find pallet file %s, returning nullptr",
    file_name.c_str()
  );
  return nullptr;
}

void PalletManager::addPalletFileToMap(
  const std::string &pallet_file,
  std::string ztd_file
) {
  this->pallet_files_map[pallet_file] = ztd_file;
}

void PalletManager::loadPalletMap(
...
void PalletManager::loadPallet(const std::string &file_name) {
  ...
  void *pallet_file_content = ZtdFile::getFileContent(
    this->pallet_files_map[file_name],
    file_name,
    &pallet_file_size
  );
  ...
  if (pallet_file_content == nullptr) {
    SDL_Log(
      "Could not load pallet %s from %s, returning",
      file_name.c_str(),
      this->pallet_files_map[file_name].c_str()
    );
    return;
  }
