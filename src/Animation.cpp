...
void Animation::draw(SDL_Renderer *renderer, int x, int y,
  ...
  if (!this->textures[direction_string].empty()) {
    SDL_QueryTexture(this->textures[direction_string][this->current_frame],
...
void Animation::draw(SDL_Renderer *renderer, SDL_Rect *dest_rect,
  ...
  if (direction_string.empty()) {
    ...
    if (!direction_string.empty()) {
      ...
      SDL_Log("Cannot draw animation because the specified direction does not "
              "exist");
      return;
    }
  }
  ...
  if (dest_rect->w == 0 || dest_rect->h == 0) {
    SDL_QueryTexture(this->textures[direction_string][this->current_frame],
                     NULL, NULL, &dest_rect->w, &dest_rect->h);
  }
  ...
  if (this->has_background) {
    SDL_RenderCopyEx(
        renderer,
        this->textures[direction_string]
                      [this->textures[direction_string].size() - 1],
        NULL, dest_rect, 0, NULL, this->renderer_flip);
  ...
  SDL_RenderCopyEx(renderer,
                   this->textures[direction_string][this->current_frame], NULL,
                   dest_rect, 0, NULL, this->renderer_flip);
}
...
static void calculateOffset(AnimationData *data, int16_t *offset_x,
  ...
  *offset_y = 0;
  ...
  for (int i = 0; i < ((int)data->frame_count + (int)data->has_background);
       i++) {
    int16_t current_frame_offset_x = (data->width / 2) -
                                     (data->frames[i].width / 2) +
                                     data->frames[i].offset_x;
    int16_t current_frame_offset_y = (data->height / 2) -
                                     (data->frames[i].height / 2) +
                                     data->frames[i].offset_y;

    bool all_frames_fit = true;
    for (int j = 0; j < ((int)data->frame_count + (int)data->has_background);
         j++) {
      if (i == j) {
        continue;
      }
      if (current_frame_offset_x - data->frames[j].offset_x < 0 ||
          data->frames[j].width + current_frame_offset_x -
                  data->frames[j].offset_x >
              data->width ||
          current_frame_offset_y - data->frames[j].offset_y < 0 ||
          data->frames[j].height + current_frame_offset_y -
                  data->frames[j].offset_y >
              data->height) {
  ...
  // Fallback: use the first frame's center-based offset
  // This handles UI button animations that may not have complex frame alignment
  ...
  *offset_y = (data->height / 2) - (data->frames[0].height / 2) +
  ...
  SDL_Log("calculateOffset: Using first frame fallback offset (%d, %d) for "
          "%dx%d animation",
          *offset_x, *offset_y, data->width, data->height);
}
      ...
      for (int instruction = 0;
        ...
        for (int p = 0;
          ...
          if (x < 0 || x >= data->width || y < 0 || y >= data->height) {
            SDL_Log("Failure to draw within the bounds. %i,%i does not fit in "
                    "a %i,%i rectangle",
                    x, y, data->width, data->height);
            // Cleanup partial surfaces
            for (auto *s : this->surfaces[direction_string]) {
              SDL_FreeSurface(s);
            }
            this->surfaces[direction_string].clear();
            return;
...
bool Animation::hasFrames(CompassDirection direction) {
  std::string dir_str = convertCompassDirectionToExistingAnimationString(
      direction, this->surfaces);
  if (!dir_str.empty()) {
    return !this->surfaces[dir_str].empty();
  }
  dir_str = convertCompassDirectionToExistingAnimationString(direction,
                                                             this->textures);
  if (!dir_str.empty()) {
    return !this->textures[dir_str].empty();
  }
  return false;
}
