...
public:
...
#ifdef _WIN32
  static std::string fixPath(std::string path_string) { return path_string; }
#else
  static std::string getCorrectCaseFilename(std::string base_path,
                                            std::string file_name) {
  ...
  static std::string fixPath(std::string path_string) {
    ...
    for (std::filesystem::path part : path) {
      ...
      std::string new_part =
          Utils::getCorrectCaseFilename(new_path.string(), part.string());
      if (new_part.empty())
        return "";
  ...
  static Expansion getExpansion() {
    if (!Utils::getExpansionLangDllPath(Expansion::MARINE_MANIA).empty() &&
        !Utils::getExpansionLangDllPath(Expansion::DINOSAUR_DIGS).empty()) {
